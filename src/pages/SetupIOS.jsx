            ...
            <ol className="mt-6 space-y-4 text-slate-300 list-decimal list-inside">
              {page.steps.map((step, index) => (
                <li key={step} className="rounded-3xl bg-slate-950/80 p-4">
                  {index === 1 ? (
                    <a href={page.download} target="_blank" rel="noreferrer" className="text-[#ffbb00] hover:text-[#e0a900] underline">
                      {step}
                    </a>
                  ) : index === 2 ? (
                    <a href="https://wg.playmetod.store" target="_blank" rel="noreferrer" className="text-[#ffbb00] hover:text-[#e0a900] underline">
                      {step}
                    </a>
                  ) : (
                    step
