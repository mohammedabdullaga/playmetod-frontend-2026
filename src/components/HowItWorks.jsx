    ...
    <section className="px-4 pb-16 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-6xl rounded-[32px] border border-[#ffbb00]/15 bg-slate-950/80 p-8 shadow-glow sm:p-10">
        <div className="grid gap-8 lg:grid-cols-[0.9fr_1.1fr] lg:items-center">
          <div>
            <p className="text-sm uppercase tracking-[0.28em] text-[#ffbb00]/90">{texts.howItWorks.title}</p>
          ...
          <div className="grid gap-4 sm:grid-cols-3">
            {texts.howItWorks.steps.map((step, index) => (
              <div key={step.title} className="rounded-[28px] border border-[#ffbb00]/15 bg-slate-900/80 p-6 text-center shadow-lg transition hover:-translate-y-1">
                <div className="mx-auto mb-4 flex h-14 w-14 items-center justify-center rounded-full bg-[#ffbb00]/15 text-xl font-semibold text-[#ffbb00]/90">
                  {index + 1}
                </div>
